...
_ANNOTATION_SAVE_PATH = 'Annotations_speedup'
...
# 创建XML文档并写入节点信息
def createXMLFile(imgname, width, height, filename):
    ...
    # filename节点
    createChildNode(doc, 'filename', imgname, root_node)
...
if __name__ == "__main__":
    ...
    for key, v in info_dict.items():
        ...
        # 构建XML文件名称
        xml_file_name = os.path.join(_ANNOTATION_SAVE_PATH, (key.split('.'))[0] + '.xml')
        print (xml_file_name)
        if 1:
            # print('not exists')
            ...
            # 创建XML文件
            createXMLFile(img_name, width, height, xml_file_name)
